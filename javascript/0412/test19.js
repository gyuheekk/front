...
function regexCheck() {
  ...
  const regex11 = /[^a-zA-Z0-9_-]/g;        // 영문 소문자, 영문대문자, 숫자, _, - 만 포함?

  const regex21 = /\./g; // '.'을 포함?
  const regex22 = /\d/g; // 숫자 포함하고 있는가?
  const regex23 = /\D/g; // 숫자를 포함하지 않고 있는가??
  const regex24 = /\w/g; // 영문자/숫자/_/을 포함하고 있는가??
  const regex25 = /\W/g; // 영문자/숫자/_/을 포함하고 있는가??
  ...
  // if(regex11.test(content)) alert("영문소문자/대문자/숫자/_/- 이 아닌 다른 문자를 포함하고 있습니다.");
